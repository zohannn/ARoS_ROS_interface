...
{
	//obj_oor = this->obj_rpy_or;
	//obj_oor.resize(this->obj_rpy_or.size());
	//obj_oor.at(0) = this->lpf_obj_or_roll->update(this->obj_rpy_or.at(0));
	//obj_oor.at(1) = this->lpf_obj_or_pitch->update(this->obj_rpy_or.at(1));
	//obj_oor.at(2) = this->lpf_obj_or_yaw->update(this->obj_rpy_or.at(2));
...
void Object::setPos(std::vector<float>& obj_pos)
{
	...
	Vector3f obj_pppos = RotMat*obj_ppos;
...
void Object::setOr(std::vector<float>& obj_or)
{
	...
	//Matrix3f mat = q.toRotationMatrix();
	//this->obj_rot = RotMat*q.toRotationMatrix();
	this->obj_rot = RotMat*Rot;
...
void Object::setLowPassFilters(float freq, float dt)
{
	this->lpf_obj_pos_x->setCutOffFrequency(freq); this->lpf_obj_pos_x->setDeltaTime(dt);
	this->lpf_obj_pos_y->setCutOffFrequency(freq); this->lpf_obj_pos_y->setDeltaTime(dt);
	this->lpf_obj_pos_z->setCutOffFrequency(freq); this->lpf_obj_pos_z->setDeltaTime(dt);
	this->lpf_obj_qor_x->setCutOffFrequency(freq); this->lpf_obj_qor_x->setDeltaTime(dt);
	this->lpf_obj_qor_y->setCutOffFrequency(freq); this->lpf_obj_qor_y->setDeltaTime(dt);
	this->lpf_obj_qor_z->setCutOffFrequency(freq); this->lpf_obj_qor_z->setDeltaTime(dt);
	this->lpf_obj_qor_w->setCutOffFrequency(freq); this->lpf_obj_qor_w->setDeltaTime(dt);
}
