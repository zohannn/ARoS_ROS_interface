/*****************************************************************************
  ...
  Version: 1.0
  Author: Gianpaolo Gulletta <gianpaologulletta@gmail.com>
  Maintainer: Gianpaolo Gulletta <gianpaologulletta@gmail.com>
...
CircularBuffers<T>::CircularBuffers():
    m_capacity(0)
{
    m_buffers.assign(1, std::deque<T>(0));
}
...
bool CircularBuffers<T>::full()
{
    ...
    for(int i=0;i<m_buffers.size();++i) {
		std::deque<T> dq = m_buffers.at(i);
        full = full && (dq.size() == m_capacity);
    }
    return full;
}
