...
void UpperLimbReqProcessor::SetDevices(boost::shared_ptr<AmtecLWA7dof> arm_, boost::shared_ptr<BarrettHand826X> hand_)
{
	arm = arm_;
	hand = hand_;
		...
		try
		{
			...
			for each(auto &step in std::get<0>(mov))
			{
				...
				if (run_hand && soft_enabled_hand && hand && hand_machine_state != HandMachineState::Grasping)
				{
					...
					boost::function<void(std::vector<double>, boost::shared_ptr<BarrettHand826X>, boost::shared_future<void>)> move_hand =
						[](std::vector<double> hand_joints, boost::shared_ptr<BarrettHand826X> hand, boost::shared_future<void> previous_hand_handle)
					{
