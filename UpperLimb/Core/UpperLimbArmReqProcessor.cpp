...
/*
bool UpperLimbReqProcessor::ArmCheckCommand(unsigned int cmd)
{
	switch (cmd)
	{
...
bool UpperLimbReqProcessor::ArmProcessor(CMessage &in_req, CMessage &out_reply, YarpConnector &con)
{
	int
		nJoints = 0,
		returned_code = 0,
...
void UpperLimbReqProcessor::HomeJoint(int joint)
{
	if (!arm->HomeJoint(joint))
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::Home(std::vector<unsigned int> &home_order)
{
	if (!arm->Home(home_order))
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::HomeAll(void)
{
	if (!arm->HomeAll())
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::HomeGripper(void)
{
	if (!arm->HomeGripper())
		throw arm->Get_Error_Str();
}

std::string UpperLimbReqProcessor::Get_Device_Error(void)
{
	return arm->Get_Error_Str();
}

bool UpperLimbReqProcessor::isArmStatusOK(void)
{
	return arm->isArmStatusOK();
}

void UpperLimbReqProcessor::freeJoint(int nJoint)
{
	if (!arm->freeJoint(nJoint))
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::ResetJoint(int joint)
{
	if (!arm->Reset(joint))
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::ResetAll(void)
{
	if (!arm->ResetAll())
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::Restart()
{
...
void UpperLimbReqProcessor::StopJoint(int joint)
{
	if (!arm->stop(joint))
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::StopAll(void)
{
	if (!arm->stop())
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::SoftStop(void)
{
	if (!arm->SoftStop())
		throw arm->Get_Error_Str();
}
...
int UpperLimbReqProcessor::GetNumberJoints(void)
{
	int n = 0;
	if (!arm->getAxes(&n))
		throw arm->Get_Error_Str();
...
float UpperLimbReqProcessor::GetPos(int joint)
{
	float fAux = 0.0f;
...
void UpperLimbReqProcessor::GetPosAll(std::vector<float> &data)
{
	int number = 0;
...
float UpperLimbReqProcessor::GetPosDeg(int joint)
{
	float fAux = 0.0f;
...
std::vector<float> UpperLimbReqProcessor::GetPosDegAll(void)
{
...
std::vector<float> UpperLimbReqProcessor::Get_Force_Torque(void)
{
	std::vector<float> fData;
...
void UpperLimbReqProcessor::GetJointLimits(int joint, float * fLimits)
{
...
int UpperLimbReqProcessor::WaitMotionEnd(unsigned int timeout_ms)
{
	int state = 0;
...
void UpperLimbReqProcessor::WaitMotionEnd(std::vector<float> &refs, float stop_margin)
{
	if (!arm->waitMotionEnd(refs.data(), stop_margin))
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::MoveJointPosition(int joint, float value)
{
	if (!arm->positionMove(joint, static_cast<double>(value)))
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::MoveJointPositionRamp(int joint, float pos)
{
	if (!arm->positionMoveRamp(joint, pos))
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::MoveJointPositionRamp(int joint, float pos, float vel, float acc)
{
	if (!arm->positionMoveRamp(joint, pos, vel, acc))
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::MoveStep(int joint, float value, unsigned short time, bool wait_end)
{
	if (!arm->positionMove(joint, value, time, wait_end))
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::MoveStepAll(std::vector<float> &fData)
{
...
void UpperLimbReqProcessor::MoveStepSameTime(std::vector<float> *fData, std::vector<unsigned int> *joints2move)
{
...
void UpperLimbReqProcessor::MoveStepSameTimeWait(std::vector<float> &fData)
{
	if (!arm->positionMove(fData.data(), static_cast<unsigned short>(fData.back()), true))
		throw arm->Get_Error_Str();
}

void UpperLimbReqProcessor::MoveStepAllSameTime(std::vector<float> &fData)
{
